...
class Dictionary {
    ...
    lookup(searchTerm) {
        ...
        const successful = R.pipe(
            R.filter(isSuccess),
            R.head,
            result => isKanji(result.value) ? this._withWords(result) : result,
        )(results)
    ...
    _withWords(kanjiResult) {
        const wordsResult = this._kanjiapi
            .getWordsForKanji(kanjiResult.value.kanji)

        return {
            status: wordsResult.status === LOADING ? LOADING : SUCCESS,
            value: wordsResult.status === LOADING ? null : {
